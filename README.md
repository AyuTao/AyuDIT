...
-   **多时间线选择 + 拖拽排序**：按所选顺序生成报告。（当你用每张卡卷号来新建时间线的时候，该时间线就是当前卷号卡的全部内容统计）
-   **批量导出视频**：可多选时间线进行批量导出，也可以为时间线选中多个预设格式进行导出。
...
-   **单帧缩略图导出**：
    ...
    -   文件命名：`时间线名称-素材文件名-时间码.jpg`（无法获取素材文件名时回退为 `时间线名称-时间码.jpg`）
...
-   **刷新与自动刷新**：头部刷新按钮 + 自动刷新（默认 300 秒），同时更新时间线列表与仪表盘数据。
 -   **多轨时间线处理（重要）**：
     -   默认按“合成输出”的可见结果抓取缩略图/单帧，即抓到的是“当前时间点最上层可见轨道”的画面。
     -   对跨轨道的“同一素材同一片段”自动去重，避免重复抓取/导出。
     -   为了结果一致、可预测，要求（建议）素材仅放在一个视频轨道，不要在同一时间段多轨堆叠同一素材或遮挡。
...
多轨时间线规则：PDF 中的缩略图按“最上层可见”的合成画面生成；若同一素材片段同时出现在多个轨道且彼此重叠，会自动去重，仅计一次缩略图与统计。
...
多轨时间线规则：
-   默认导出的是时间线“合成输出”的可见画面，即当前时间点最上方可见轨道的内容；若有字幕/图形叠加，也会一并出现在导出的单帧中。
-   对跨轨相同素材相同片段自动去重，仅导出一次，避免重复。
-   导出文件名包含素材文件名，便于快速识别来源；若无法确定素材（例如标记位于空帧），会回退为不带素材名的命名。
-   为了避免被上层轨道遮挡导致不符合预期，要求素材仅放在一个视频轨道；如需输出“素材自身”的干净画面，请确保导出帧时该素材处于最上方且无遮挡。
...
-   不确定时，中间帧通常是最稳定的缩略图选择。
 -   插件不会修改达芬奇轨道可见/静音状态，导出的画面来自当前时间线的合成结果；如需严格的“素材自身”画面，请按上文的单轨/最上层规范整理时间线。
